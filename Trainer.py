...
class Trainer(object):
    ...
    #이부분 변경하기
    def load_data(self, file_name, loader_name, is_test=False): #csv불러와서 padding?
        loader_file = os.path.join(self.dataset_dir, loader_name) #텐서 저장경로. test.pt가 그냥 데이터텐서임. dataset_dir=./data
        if os.path.exists(loader_file):
            ...
            with open(os.path.join(self.dataset_dir, file_name), 'r', encoding='utf-8') as f:
                ...
                for line in tqdm(r):
                    src = line[0].strip()
                    print("src: ",src) #src:  article,highlights,id
                    print("line: ",line) #line: ['article,highlights,id']
                    # add preprocess progress
                    #src = toSimpleChinese(src) #not needed => 중국어 간체자로 바꾸는작업 
                    src_data.append(src)
                    if not is_test:
                        #trg=line.strip()
                        print(line)
                        #trg = line[1].strip() #target
        ...
        if os.path.exists(loader_file):
            ...
            with open(os.path.join(self.dataset_dir, file_name), 'r', encoding='utf-8') as f:
                ...
                for line in tqdm(data):
                    # add preprocess progress
                    src =line.strip() #toSimpleChinese(line.strip()) #이 부분을 그냥 data.strip()으로 바꿔야할듯
                    src_data.append(src)
            '''
    ...
    def train(self):
        ...
        # self.train_data = self.load_data(args.train_file, 'train.pt', is_test=False)=>csv불러오기, train_file=train.csv
        # def load_data(self, file_name, loader_name, is_test=False) loader_name은 저장용
